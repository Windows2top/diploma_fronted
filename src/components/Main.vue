<template>
  <header class="bg-white rounded-lg w-auto shadow-lg">
    <h1 class="text-4xl sm:text-5xl lg:text-7xl font-bold text-green-400 p-2">
  ...
  <main class="py-7 min-h-[50vh]">
    <div class="w-full">
    <div v-if="tests.length === 0" class="flex justify-center items-center w-full py-8">
      <svg-icon type="mdi" :path="mdiLoading" class="animate-spin text-gray-500 size-20" />
    </div>

    <div v-else class="flex flex-wrap justify-between gap-8">
      <div
